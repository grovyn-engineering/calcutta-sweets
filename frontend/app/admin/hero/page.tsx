...
export default function HeroAdminPage() {
  ...
  if (loading) return <div className="text-[#3E2F26]/40 text-sm animate-pulse">Loading hero slides...</div>;
  if (error) return <div className="text-red-500 bg-red-50 p-4 rounded text-sm mb-4">Error loading slides: {error}</div>;
    ...
    <div className="max-w-4xl">
      ...
      <div className="bg-white border border-[#3E2F26]/10 rounded-lg shadow-sm p-6 mb-8">
        ...
        {(!slides || slides.length === 0) ? (
          <p className="text-xs text-[#3E2F26]/40 italic">No custom slides added yet. Using UI fallbacks.</p>
        ) : (
          <div className="space-y-4">
            {slides.map((slide) => (
              <div key={slide.id} className="flex gap-4 items-center border border-[#3E2F26]/10 rounded p-4 group hover:border-[#C8773A]/30 transition-colors">
